...
<?php
try {
  if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST['type'] == "product") {
    authorize();
    $qty = $_POST['qty'] ?? 1;
    $size = $_POST['size'] ?? 'M';
    $color = $_POST['color'] ?? 'black';

    Cart::addToCart($data->id, $qty, $size, $color);
...
try {
  if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST['type'] == "comment") {
    ...
    $commentModel->product_id = $data->id;
    $commentModel->user_id = $auth->user_id;
    $commentModel->comment = $_POST['comment'];
    ProductComment::insert($commentModel);
  }
} catch (\Throwable $th) {
  var_dump($th->getMessage());
}
...
<?php
$product_images = ListProductImage::findAllByProductId($data->id);
$comments = ProductComment::findAllByProductId($data->id);
?>
